...
_DEFAULT_POOLS: Dict[str, Dict[str, List[str]]] = {
    WARMTH_COMFORT: {
        ...
        "center": [
            ...
            "soft and easy",
            "letting the warmth find you",
            "comfortable just like this",
            "the ease of being held",
            "nothing to do but feel this",
            "wrapped in something gentle",
            "warm enough to let go",
        ],
    },
    IDENTITY: {
        "shadows": [
        ...
        "center": [
            "you are soft, you are safe",
            "this is yours",
            "belonging here completely",
            "you are exactly this",
            "already enough",
            "nothing to prove, nothing to fix",
            "the softness is who you are",
            "claimed by your own quiet",
        ],
    },
    DISSOLUTION: {
        "shadows": [
        ...
        "center": [
            ...
            "no edges left to find",
            "the boundary was never real",
            "less and less of anything",
            "already gone before you noticed",
    ...
    GROUNDING_TEXTURE: {
        ...
        "center": [
            ...
            "notice the pressure where you rest",
            "solid and grounded",
            "the texture of this moment",
            "feel where you meet the surface",
            "anchored by your own weight",
            "gravity has you",
            "held by what's underneath",
        ],
    },
    SOMATIC_ANCHORING: {
        "shadows": [
        ...
        "center": [
            ...
            "the heaviness is a gift",
            "letting the weight carry you down",
            "dense and still and heavy",
            "deeper by the weight of it",
            "gravity pulling you under",
        ],
    },
    STILLNESS_EMPTINESS: {
        "shadows": [
...
class SemanticSelector:
    ...
    def select_pool(
        ...
        theta: float,
        alpha: float,
        conductor_phase: str = "MAINTENANCE",
    ) -> Optional[str]:
        """
        ...
        if target not in eligible:
            ...
            same_valence = [
                ...
                if (
                    valence == "approach"
                    and p in [WARMTH_COMFORT, IDENTITY, DISSOLUTION]
                )
                or (
                    valence == "withdrawal"
                    and p in [GROUNDING_TEXTURE, SOMATIC_ANCHORING, STILLNESS_EMPTINESS]
                )
            ]
            fallback = (
                same_valence[0] if same_valence else (eligible[0] if eligible else None)
            )
            if fallback is None:
                return None
            target = fallback
    ...
    def tick(self, live_state: dict) -> Optional[Dict]:
        ...
        if target_pool != self.current_pool_id:
            time_in_current = (
                now - self.pool_entered_at if self.current_pool_id else 999.0
            )
            if self.current_pool_id is not None and time_in_current < MIN_POOL_DWELL_S:
                ...
                entry = {
                    ...
                    "trance": round(
                        float(live_state.get("eeg_trance_score", 0.0) or 0.0), 3
                    ),
                    "theta_alpha": round(
                        float(live_state.get("eeg_theta", 0.0) or 0.0)
                        / max(
                            float(live_state.get("eeg_alpha", 0.001) or 0.001), 0.001
                        ),
                        3,
                    ),
                }
        ...
        if cascade:
            ...
            cascade["trance_score"] = float(
                live_state.get("eeg_trance_score", 0.0) or 0.0
            )
            cascade["theta_alpha_ratio"] = round(
                float(live_state.get("eeg_theta", 0.0) or 0.0)
                / max(float(live_state.get("eeg_alpha", 0.001) or 0.001), 0.001),
                3,
            )
    ...
    def _build_cascade(self, pool_id: str, live_state: dict) -> Optional[Dict]:
        ...
        if center_phrase and self._habituation is not None:
            self._habituation.on_stimulus_presented(
                center_phrase, "word", "center_text"
            )
    ...
    def get_state_summary(self) -> Dict:
        ...
        return {
            ...
            "pool_dwell_s": round(now - self.pool_entered_at, 1)
