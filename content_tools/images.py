...
def build_conditioning_prompt(
    ...
    style_word = (
        "an anime illustration with clean cel shading"
        if style == "anime"
        else "a photorealistic photograph"
    )
    ...
    tag_detail = ""
    unique_tags = [
        t for t in (tags or []) if t.replace("_", " ").lower() not in theme.lower()
    ]
    if unique_tags:
...
def _generate_koboldcpp(
    ...
    try:
        ...
        raise RuntimeError(
            f"KoboldCpp txt2img response is not JSON ({exc}). First bytes: {snippet!r}"
        ) from exc

    images = data.get("images")
    if not images:
        raise RuntimeError(f"KoboldCpp returned no images. Response: {str(data)[:300]}")
...
def generate_and_tag(
    ...
    **kwargs,
    ...
    """Generate n images and register each one in somna.db under the given tag.
