...
class IMUEngine:
    ...
    def tick(self, board: object, board_id: int) -> dict:
        ...
        try:
            ...
            try:
                _override = json.loads(_LIVE.read_text(encoding="utf-8")).get(
                    "imu_motion_threshold_override"
                ) if _LIVE.exists() else None
                motion_thresh = float(_override) if _override is not None else _MOTION_THRESH
            except Exception:
            ...
            self._stillness_ema = (
                (1.0 - _STILLNESS_ALPHA) * self._stillness_ema
                + _STILLNESS_ALPHA * raw_stillness
            )
