...
Written to live_control.json via patch_live():
...
class PACEstimator:
    ...
    ISA_WINDOW_SEC = (
        30.0  # ISA needs more data; use full available buffer when possible
    )
    ...
    def _compute_isa_alpha(self, long_buffer: np.ndarray, iaf: float) -> float:
        ...
        if len(long_buffer) < self.isa_window_len:
            ...
            if len(long_buffer) < self.window_len:
                return 0.0
            return self.compute_mi(
                long_buffer[-self.window_len :],
    ...
    def compute_all(
        self, eeg_buffer: np.ndarray, iaf: float = 10.0, now: float = 0.0
    ) -> dict:
        """
        ...
        Returns dict of pac_* keys ready for patch_live().
        """
        # Rate limiting
