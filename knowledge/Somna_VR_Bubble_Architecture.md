...
2. Luminance rises gradually, not overhead but from the periphery, as if dawn is seeping through the membrane.
...
| WORK_WINDOW | 2.2–2.6m | 0.6–0.9 | 2000K (blue-black) | 0.05–0.1 |
...
|---|---|---|---|
| W1 | Bubble geometry + void register + horizon element + spatial carrier + ambient drone + smoke fill (§22.1) | Ship | Enclosing darkness with a breathing ring, beat-synchronized contraction, audio from the walls, soft haze giving the space depth |
| — | **Validation gate:** run real sessions; confirm headset deepens the work | Must pass before W2 | |
| W2 | Membrane opacity + content projection (text quads, images on inner surface) + color temp curve driven by phase + breathing sync options + transition designs (§23) | W1 validated | Affirmations floating just inside the walls, images pressed onto the membrane, color shifting with depth, every phase transition felt but not consciously noticed |
...
Currently audio lives in the desktop process; it must stay alive for any session. If VR-only sessions (no desktop) are ever desired, audio needs its own lightweight subprocess. Don't design around this assumption yet — just don't close the door architecturally.
...
This connection should be made explicit in the ImGui panel: the Interference Graph's spread parameter could optionally drive `vr_bubble_breathing_pct`, so that composing a wider chord automatically intensifies the somatic pressure. The user *feels* the chord they composed, not just hears and sees it.
...
Implementation: on exit, `vr_display_runner.py` writes current bubble parameters to `user_settings.json` under a `vr_bubble_last_state` key. On startup, it reads these and uses them as initial values instead of hardcoded defaults, unless `vr_safety_kill` was True at exit.
