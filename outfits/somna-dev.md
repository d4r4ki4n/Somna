...
| `tools/mcp_somna_server.py` | MCP server exposing Somna runtime state to LLM clients (Kilo, Cursor, etc.) via stdio. Phase 1: 9 read-only tools. Phase 2: 4 scoped write tools. Phase 3: TCP :6790 prompt bridge + MCP `sampling/createMessage`. Registered in `.kilo/kilo.json` under `"mcp": { "somna": ... }`. Read tools: `somna_read_live`, `somna_read_conductor`, `somna_read_eeg`, `somna_query_db`, `somna_list_sessions`, `somna_read_profile`, `somna_tail_decisions`, `somna_read_session_yaml`, `somna_read_affirmations`. Write tools: `somna_patch_live` (28 whitelisted keys with range validation), `somna_update_profile` (append-only paths + settable scalars), `somna_append_affirmations` (line append), `somna_write_conductor_hint` (depth_patience, request_fractionation, target_floor_hz, note, passthrough), `somna_write_agent_response` (structured LLM response dict written to `agent_ext_response` key in live_control.json; keys: response, adjustments, transitions, action, next_prompt, prompt_style, next_affirmation, reasoning, session, session_intent). Blocking tools: `somna_read_live_blocking` (sleep N seconds then return live state), `somna_wait_for` (poll until a live_control.json key matches expected value, or timeout; operators: eq/neq/gte/lte/gt/lt/contains), `somna_prompt_user` (write agent_message with needs_response=true, block until user_response arrives or timeout; voice_mode: tts/subliminal/both/silent). Session lifecycle tools: `somna_launch_session`, `somna_stop_session`. All tool responses include `_temporal` metadata: session_elapsed_sec, since_last_call_sec, calls_this_session, calls_per_minute_rolling, conductor_phase, conductor_phase_elapsed_sec, session_playback_sec. Phase 3: `_PromptBridge` accepts TCP connections from `somna_agent.py`, forwards prompts to the MCP client via `session.create_message()`, returns LLM responses over TCP. `SomanaMCPServer` subclasses the low-level MCP Server to capture the session reference and start the bridge as a background task. Design spec: `knowledge/mcp_server_architecture.md`. |
| `tools/external_agent_client.py` | Sync TCP client used by `somna_agent.py` to push prompts to the MCP prompt bridge on TCP :6790. Persistent socket connection with background reader thread. `request(prompt, system_prompt, tick_id)` sends prompt and blocks for response (default 30s timeout). Falls back gracefully — agent uses local LLM when bridge unavailable. Config: `agent.external_channel: true` in `agent_config.yaml`. When `external_only: true`, `LLMClient.chat()` is the single routing point — returns empty string on external failure instead of falling through to local LLM. |
...
| `ipc/__init__.py` | Re-exports `patch_live`, `write_live`, `StateServer`, `PORT` |
...
| `ui/__init__.py` | `ui` package — Dear ImGui panel helper modules |
...
3\. Add init/update/draw dispatch in the `__init__ / `update` / `draw` methods
...
When present, `dispatch(tool_name, args)` is called from `content_tools/__init__.py`. The result is injected back as a second LLM call so the model can make a decision with the information. Only one tool call per turn. The loop does not recurse — if the second response also contains `tool_call`, it is ignored.
...
**`pygame.mixer.pre_init(44100, -16, 2, 512)`** must be called before `pygame.init()` and is done in `control_panel_imgui.py` at startup. Do not move it into `BinauralAudioEngine.__init__ — the mixer is already initialised by the time the engine is created.
...
**Entry point** — use `python visual_display_runner.py --vr`. Running `python visual_display.py --vr` directly does nothing useful — `visual_display.py` has no `__main__ block.
...
**Oklab color space** is available in `common.glsl` as `rgbToOklab()`, `oklabToRgb()`, `mixOklab()`. Not yet used by any style — ready for Phase 2 color pass integration.
...
- Do not change the init order in `VisualDisplay.__init__ — VR init happens after `_open_window()` because the GL context must exist first
...
- Do not run `python visual_display.py --vr` directly — there is no `__main__ block; the entry point is `visual_display_runner.py`
...
- When two behaviors were incorrectly conflated (e.g. one log level for both streams), the user expects a proper split into distinct code paths (separate visibility, source, or ownership), not a cosmetic workaround.

## Learned Workspace Facts

- The project was previously named **HypnoSlide** before being renamed to **Somna**; the ported HypnoSlide transcript is `f585d737-7913-4141-8793-c8b5913dfd70` in agent-transcripts.

- `ui/panel_theme.py` `token_rgba(name)` maps **semantic** names via `COLOR_TOKENS` (e.g. `text_primary`, `panel_bg`, `source_user_lock`). Do not pass legacy raw `RP` key strings like `"text"` or `"iris"` into `token_rgba`. Use `RP[...]` / `hex_to_rgba` only where raw palette tuples are required (e.g. Tk, hand-built `ImVec4`).

- ImGui bundle reminders: `ChildFlags_.borders` (not `border`), `SelectableFlags_.none`, `HoveredFlags_.delay_short`, `WindowFlags_.no_resize`. When unsure of a flag symbol, introspect with `python -c "from imgui_bundle import imgui; print([x for x in dir(imgui.ChildFlags_) if not x.startswith('_')])"` (swap `ChildFlags_` for the enum you need).

- Root helpers: `peek.py` / `patch.py` for inspecting or patching `live_control.json`; `smoke_test.py` runs ~153 automated checks (Bible Ch.2–Ch.8). `visual_sweep.py`: lines starting with `notes ` are stored and passed into `_save_snapshot(..., user_notes)` — not discarded.

- TMR pool names in `tmr_cue_manager.py` are **UPPERCASE** (`IDENTITY`, `RELEASE`, `POTENTIAL`, `SOMATIC`, `PURPOSE`, `TRANSITION`); lowercase strings silently map to `POOL_SIGNATURES["IDENTITY"]`.

- `sessions` table: `name` is TEXT PRIMARY KEY. Other tables use `session_id TEXT NOT NULL` without FK — keep that pattern for new tables.

- **Interference Graph / Somatic Palette Mixer:** `ui/interference_graph.py` (model), `ui/interference_graph_panel.py` (ImGui), `ui/interference_graph_integration.py` (wire `install_interference_graph(panel_manager)` once). Drag state lives on the panel instance; call `imgui.reset_mouse_drag_delta` when starting a drag frame. **Spread** adds per-channel offset on top of each node’s base frequency (not a shared center). Tether Bezier “bow” scales with distance (no fixed bow constant). **Glow** stroke (wide, low alpha) draws **before** the solid tether stroke. Haptic/VNS unavailable: nodes drawn dimmed and non-draggable but still modeled.

- `ControlPanelManager.set_section_extra(section, fn)` plus `sections.setdefault(_sec, [])` in `_render_sections` / `_render_spill_sections` so canvas-only sections get a header and paint without a dummy `panel_config.json` widget.

- **`ControlPanelManager.render()` is unused** — real UI is hello_imgui `DockableWindow` handlers and `SessionPlayer`. **Console:** `SpectrogramConsole` splits **INFO** rows with `_show_info_user` / `_show_info_system` by `LogEntry.source` (`"you"` vs system); user-typed console lines should use `info(..., src="you")`; filter chip colors use `log_info_user_rgba_f` / `log_info_system_rgba_f` from `ui/console.py`.

- **Sessions dock:** seek bar + pause/next in `SessionPlayer._draw_seek_bar()`; `_seeking` blocks live slider overwrites while dragging; `_launch_display_for()` calls `_stop_display()` before switching sessions. **Playlist:** timeline runner owns `playlist_index` (writes to live for UI); `_sync_playlist_from_live` does not sync the index back; `_sync_playlist_to_live` writes `playlist` + `playlist_mode` only.

- **`timeline_paused`:** `visual_display.py` clears to transparent and skips drawing while paused. `control_panel_imgui.py` `_poll_audio()` uses `session_active = self._is_running() and not timeline_paused` so pool TTS pauses; agent one-shot / priority prompts still run.

- **OpenXR headset path** (`vr/vr_display_runner.py`): current stack is primarily **binocular** (per-eye luminance, flicker, rivalry, dichoptic SSVEP, full-field Ganzfeld)—scientifically “two eyes, one brain,” not a full volumetric scene. Vection and subliminal depth planes add **thin** 3D layering, not Skyrim-scale worlds.