...
vec4 style_interference(vec2 p) {
    ...
    return vec4(col, g * u_opacity * 1.0 * 0.85) * entrainmentModulation();
}
...
vec4 style_electric(vec2 p) {
    ...
    return vec4(col, (arm + spark) * u_opacity * 1.0) * entrainmentModulation();
}
...
vec4 style_vortex(vec2 p) {
    ...
    return vec4(col, (g + core * 0.45) * u_opacity * 1.0) * entrainmentModulation();
}
...
vec4 style_spirograph(vec2 p) {
    ...
    return vec4(col, g * u_opacity * 1.0 * smoothstep(0.0, 0.04, r)) * entrainmentModulation();
}
...
vec4 style_fermat(vec2 p) {
    ...
    float alpha = (arm + core_glow * 0.4) * u_opacity * 1.0;
    return vec4(col, alpha) * entrainmentModulation();
}
