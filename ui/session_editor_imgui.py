...
_BOOL_PARAMS: set[str] = {
    "audio_muted", "center_flash_sync_to_beat", "tts_enabled",
    "tts_subliminal", "window_always_on_top", "window_click_through",
    "spiral_show_text", "beat_sync_master", "start_fullscreen",
}

_STR_PARAMS: dict[str, list[str]] = {
    "spiral_style":     ["tunnel_dream", "galaxy", "archimedean", "kaleidoscope",
                         "interference", "electric", "vortex", "dna", "fibonacci",
                         "rose", "moire", "spirograph", "fermat", "superformula",
                         "liminal", "resonant", "nebula", "bifurcate"],
...
_CATEGORY_OPTIONS= ["general", "focus", "sleep", "entrainment", "genus", "edison", "ssild", "custom"]

# Keys never useful as session-yaml params
_SKIP_CAPTURE: set[str] = {
    "session_time", "session_duration", "session_name", "session_folder",
    "timeline_label", "timeline_paused", "timeline_locked_params",
    "playlist_index", "playlist", "playlist_mode",
    "_timeline_cmd", "seek_time",
    "agent_message", "user_response", "response_timestamp",
    "user_console_input", "user_console_ts", "agent_console_response",
    "agent_mode",
    # legacy
    "llm_prompt", "llm_prompt_written_at", "llm_prompt_style",
    "llm_prompt_timeout_s", "tts_prompt",
}
...
class SessionEditorModal:
    ...
    def open(self, session_path: Optional[Path], live_state: dict | None = None) -> None:
    ...
    def _load_yaml(self, path: Path) -> None:
        try:
            ...
            self._cat_idx  = _CATEGORY_OPTIONS.index(cat) if cat in _CATEGORY_OPTIONS else 0
            self._defaults = dict(data.get("defaults") or {})
            for kf in (data.get("timeline") or []):
                self._keyframes.append(_KF(
                    t=float(kf.get("t", 0)),
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            yaml_text = yaml.dump(
                out, allow_unicode=True, sort_keys=False, default_flow_style=False,
            )
            (self._session_path / "session.yaml").write_text(yaml_text, encoding="utf-8")
            self._dirty = False
    ...
    def _capture(self, live: dict) -> None:
        ...
        for k, v in live.items():
            if k in _SKIP_CAPTURE:
                continue
            if k.startswith("_") or k.startswith("eeg_") or k.startswith("ppg_") \
               or k.startswith("imu_") or k.startswith("sleep_") or k.startswith("tmr_") \
               or k.startswith("tts_playing") or k.startswith("conductor_"):
                continue
            if v is None:
                continue
    ...
    def _sync_kf_bufs(self) -> None:
        ...
        if 0 <= self._sel_kf < len(self._keyframes):
            ...
            ease = kf.ease
            self._kf_ease_idx  = _EASE_OPTIONS.index(ease) if ease in _EASE_OPTIONS else 0
    ...
    def _render_timeline(self, avail_w: float, live: dict) -> None:
        ...
        try:
            ...
            dur = max(60.0, (self._keyframes[-1].t * 1.15 + 15)) if self._keyframes else 1800.0
        ...
        while t <= dur + 0.01:
            tx = t_to_x(t)
            dl.add_line(imgui.ImVec2(tx, axis_sy - 4), imgui.ImVec2(tx, axis_sy + 4), C_TICK, 1.0)
        ...
        # Session end marker
        xe = t_to_x(dur)
        dl.add_line(imgui.ImVec2(xe, axis_sy - 8), imgui.ImVec2(xe, axis_sy + 8), C_END, 2.0)

        # Playhead (gold triangle + vertical line)
        ph_t = float(live.get("session_time") or 0.0)
        ph_x = t_to_x(ph_t)
        dl.add_line(imgui.ImVec2(ph_x, pos.y + 4), imgui.ImVec2(ph_x, axis_sy - 1), C_PLAY, 2.0)
        ...
        for i, kf in enumerate(self._keyframes):
            ...
            if sel:
                ...
                pts = [(kx, axis_sy - R), (kx + R, axis_sy),
                       (kx, axis_sy + R), (kx - R, axis_sy)]
                for j in range(4):
                    x0, y0 = pts[j]
                    x1, y1 = pts[(j + 1) % 4]
                    dl.add_line(imgui.ImVec2(x0, y0), imgui.ImVec2(x1, y1),
                                imgui.IM_COL32(255, 255, 255, 160), 1.5)
    ...
    def _render_body(self, avail_w: float) -> None:
        ...
        # Left: keyframe list
        imgui.begin_child(
            "##kflist", imgui.ImVec2(left_w, body_h),
        ...
        for i, kf in enumerate(self._keyframes):
            ...
            sel, _  = imgui.selectable(
                f"{display}##kf{i}", self._sel_kf == i,
                imgui.SelectableFlags_.none, imgui.ImVec2(0, 0),
            )
        ...
        imgui.begin_child(
            "##kfdetail", imgui.ImVec2(right_w, body_h),
            child_flags=imgui.ChildFlags_.borders,
        )
        if 0 <= self._sel_kf < len(self._keyframes):
    ...
    def _render_kf_detail(self, panel_w: float) -> None:
        ...
        chg, self._kf_ease_idx = imgui.combo("##kfease", self._kf_ease_idx, _EASE_OPTIONS)
        if chg:
