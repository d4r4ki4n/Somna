...
class SessionPlayer:
    ...
    def selected(self) -> Optional[SessionEntry]:
        return self._sessions[self._selected] if 0 <= self._selected < len(self._sessions) else None

    @property
    def playing(self) -> Optional[SessionEntry]:
        return self._sessions[self._playing] if 0 <= self._playing < len(self._sessions) else None
    ...
    def render(self, width: float = -1, height: float = -1) -> None:
        # The left column now owns all scrolling — session player renders at
        # natural height with no forced clip.  Everything below the transport
        # pushes content downward; the parent column scrolls if needed.
        ...
        # Playlist height: show all filtered sessions without internal scrolling.
        # The left column is the scroll container.
        filtered   = self._filtered_sorted()
        playlist_h = max(60.0, len(filtered) * _ROW_H + 8)
        self._draw_playlist_h(w, playlist_h)

        self._draw_description(w)
        self._draw_actions(w)
        self._draw_transport(w)

        # Queue panel — grows below the transport when sessions are queued
        if self._queue:
            imgui.spacing()
            imgui.separator()
            imgui.spacing()
    ...
    def _draw_header(self, w: float) -> None:
        ...
        dl.add_rect_filled(p, imgui.ImVec2(p.x + w, p.y + h),
                           imgui.IM_COL32(35, 33, 54, 255), 3.0)
        ...
        if self._state == SessionState.RUNNING and self.playing:
            ...
            if dur_str:
                dl.add_text(imgui.ImVec2(p.x + w - imgui.calc_text_size(dur_str).x - 8,
                                         p.y + 19), sub, dur_str)
        else:
            sub = imgui.color_convert_float4_to_u32(_SUBTLE)
    ...
    def _draw_filter_sort(self, w: float) -> None:
        ...
        sort_idx  = sort_opts.index(self._sort_mode) if self._sort_mode in sort_opts else 0
        imgui.set_next_item_width(half)
    ...
    def _draw_col_headers(self, w: float) -> None:
        ...
        dl.add_text(imgui.ImVec2(p.x + w - imgui.calc_text_size("Dur").x - 90, p.y), sub, "Dur")
        dl.add_text(imgui.ImVec2(p.x + w - imgui.calc_text_size("Category").x - 6, p.y), sub, "Category")
        imgui.dummy(imgui.ImVec2(w, h))
    ...
    def _draw_description(self, w: float) -> None:
        desc = ""
        if self.selected:
        ...
        # Compute wrapped text height so the box grows to fit all the text.
        pad = 10.0
        ...
        imgui.push_style_color(imgui.Col_.text,     _SUBTLE)
        imgui.begin_child("##sess_desc", imgui.ImVec2(w, box_h),
    ...
    def _draw_queue(self, w: float) -> None:
        ...
        # Header row
        p = imgui.get_cursor_screen_pos()
        dl.add_rect_filled(p, imgui.ImVec2(p.x + w, p.y + 22),
                           imgui.IM_COL32(35, 33, 54, 220), 3.0)
        dl.add_text(imgui.ImVec2(p.x + 6, p.y + 4), sub_u32,
        ...
        for qi, qs in enumerate(self._queue):
            is_next = (qi == 0)
            bg = imgui.color_convert_float4_to_u32(_HL_LOW if is_next else imgui.ImVec4(0, 0, 0, 0))
            imgui.push_style_color(imgui.Col_.header,         _HL_LOW)
            imgui.push_style_color(imgui.Col_.header_hovered, _HL_MED)
            clicked = imgui.selectable(f"##q{qi}", is_next,
                                       imgui.SelectableFlags_.none,
                                       imgui.ImVec2(w - 4, _ROW_H))
            if isinstance(clicked, tuple):
                clicked = clicked[0]
            ...
            if nw > available_w:
                while row_txt and imgui.calc_text_size(row_txt).x > available_w - imgui.calc_text_size("…").x:
                    row_txt = row_txt[:-1]
                row_txt += "…"
            dl.add_text(imgui.ImVec2(rmin.x + 4, ty), name_col, row_txt)

            if clicked and imgui.is_mouse_double_clicked(0):
                idx = next((i for i, s in enumerate(self._sessions) if s.name == qs.name), -1)
                if idx >= 0:
                    self._start(idx)
                    self._queue.pop(qi)
                    self._notify_queue_change()
    ...
    def _filtered_sorted(self) -> list[tuple[int, SessionEntry]]:
        ...
        for i, s in enumerate(self._sessions):
            if self._filter_cat not in ("All", "") and s.category != self._filter_cat:
                continue
            if query and query not in s.name.lower() and query not in s.category.lower():
                continue
            result.append((i, s))
