...
if __name__ == "__main__":
    ...
    # Read session name from live_control.json.
    # Do NOT fall back to "default" — that silently loads a heavy image folder
    # for sessions that have none.  Only use "default" if the directory exists.
    try:
        live = json.loads((root / "live_control.json").read_text(encoding="utf-8"))
    ...
    if not session_name or not (root / "sessions" / session_name).exists():
        ...
        print(f"[Launcher] session_folder missing/invalid — falling back to '{session_name}'")
