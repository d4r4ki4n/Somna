...
class VectionRenderer:
    ...
    def __init__(self, n_particles: int = 800):
        ...
        self._horizon[:, 0] = np.random.uniform(-_XY_SPREAD * 2, _XY_SPREAD * 2, n_horizon)
        self._horizon[:, 1] = np.random.uniform(-_XY_SPREAD * 2, _XY_SPREAD * 2, n_horizon)
    ...
    def update_cfg(self, cfg: dict) -> None:
        ...
        cap       = _SESSION_SPEED_CAP.get(
            min(self._session_count, 4), _SESSION_SPEED_CAP[4])
    ...
    def render_eye(self, eye_index: int, viewport_w: int, viewport_h: int) -> None:
        ...
        GL.glLoadIdentity()
        _perspective(self._fov_deg,
                     viewport_w / max(viewport_h, 1),
                     z_near=0.1, z_far=_Z_FAR + 10.0)
        ...
        for x, y, z in self._pos:
            ...
            GL.glVertex3f(float(x), float(y), -float(z))   # OpenGL z is into screen (negative)
        GL.glEnd()
